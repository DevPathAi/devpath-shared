...
WHERE md5(content) = '2774f115d7a678ed7b963b7ca4a326dc';
  ...
  -- 검증: 교정 대상이던 「옛 상태 3중(content md5·options·answer_key)」이 하나라도
  -- 남아 있으면 UPDATE 불발이므로 실패시킨다. 존재 여부가 아니라 잔존 여부를 재는
  -- 이유: learning-svc md2 시드 계열로 시드된 개발 DB 는 운영과 다른 변형 텍스트를
  -- 가진 행이 있어(예: 'public' 오타 유무) 「교정본 전부 존재」는 그런 환경에서
  -- 마이그레이션을 부당하게 실패시킨다. 변형 행은 옛 3중과 불일치라 여기 안 걸리고,
  -- content 무변경 재작성(29건)도 options·answer_key 교체로 3중이 바뀌어 걸리지 않는다.
  SELECT count(*) INTO matched
  FROM (VALUES
    ('8d8679ff684eac2be2b7cacbe03f00ec','["동일한 토픽에 대한 모든 메시지를 독점적으로 소비한다.","같은 그룹 내에서 각 컨슈머는 다른 파티션을 소비한다.","컨슈머 그룹은 여러 토픽의 메시지를 동시에 소비할 수 있다.","두 가지 모두 올바르지 않다."]'::jsonb,'{"correct":1}'::jsonb),
    ('100b969f41e77be41e4d04533ad071ff','["오류 발생한 파티션만 다시 처리한다.","전체 토픽에서 재시작한다.","해당 그룹의 모든 파티션이 다시 처리된다.","위치를 기억하고 이후에 다시 처리한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('2ce94d792a2efda93482ecae60d50cb2','["애플리케이션의 환경 변수를 설정한다.","스프링 빈들의 속성을 바인딩한다.","두 가지 모두 가능하다.","환경 프로파일에 따른 세부 설정을 적용할 수 있다."]'::jsonb,'{"correct":2}'::jsonb),
    ('bf225b6a9c98100c2359916968acae35','["application.properties","bootstrap.yml","config/application.yaml","logback.xml"]'::jsonb,'{"correct":2}'::jsonb),
    ('5d857d2f4df0bb89e66cdaae560902d6','["application.yml 파일에 직접적으로 활성화할 프로필 이름을 지정한다.","@ActiveProfiles 어노테이션으로 실행 시점에 프로필을 선택한다.","@Profile 어노테이션이 적용된 설정 클래스를 사용하여 프로파일별로 구분한다.","프로젝트 빌드 시 설정 파일에서 프로필을 지정해야 한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('20d2ca8643fbd9115de24181b337f442','["application.properties 파일 내부에 특정 환경을 위한 설정을 분할하고, 활성화하려는 프로필 이름으로 해당 파일을 명시한다.","스프링 부트 애플리케이션 프로퍼티를 사용하여 활성화하려는 프로파일을 명시하며, 이를 통해 특정 환경에 대한 설정만 로드된다.","application.yml 파일 내부에서 @Profile 어노테이션으로 분할된 설정을 정의하고, 필요한 프로필 이름을 지정한다.","환경 변수를 사용하여 활성화하려는 프로파일을 명시하며, 이 환경 변수에 해당하는 설정만 로드된다."]'::jsonb,'{"correct":1}'::jsonb),
    ('11f00e3b3883ae9af91fa3a7a21b4208','["환경 변수를 통해 프로파일을 동적으로 변경하는 것이 가능합니다.","application.yml 파일 내에서 모든 프로필 설정을 한 번에 적용할 수 있습니다.","프로필 기반 설정이 application.yml 파일보다 우선 순위가 높습니다.","Spring Boot는 프로필 기반 설정을 지원하지 않습니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('532fa2e6b74041401c3a0b68e320c9e6','["AuthenticationFilter","AuthenticatingFilter","UsernamePasswordAuthenticationFilter","AuthenticationProcessingFilter"]'::jsonb,'{"correct":2}'::jsonb),
    ('b4b89e5373537801bb855c1b1ed592a9','["@Entity","@Table","@JoinColumn","@OneToOne","@OneToMany"]'::jsonb,'{"correct":4}'::jsonb),
    ('a075c4156201977cd4f7f5e775adef1c','["전 처리(before), 후 처리(after) 두 가지로 전처리는 메소드 호출 이전에 작업을 수행하고, 후처리는 메소드 호출 이후에 작업을 수행한다.","전 처리(before), 후 처리(after), 예외 처리(after-throwing) 세 가지로 전처리는 메소드 호출 이전에 작업을 수행하고, 후처리와 예외 처리는 각각 메소드 호출 직후 및 예외 발생 시 작업을 수행한다.","예외 처리(around), 반환 처리(return) 두 가지로 예외 처리는 메소드 실행의 정상적인 흐름과 예외 상황 모두에 반응하며, 반환 처리는 메소드의 결과를 조작한다.","위치(pointcut)와 advice 두 가지로 위치는 특정 메소드가 호출되는 지점을 지정하고, advice는 해당 위치에서 수행할 작업을 정의한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('505882f833218806846aae379f7fa4aa','["인가는 사용자의 권한을 확인하고 인증은 로그인 여부를 확인한다.","인증은 사용자의 권한을 확인하고 인가는 로그인 여부를 확인한다.","인증은 사용자의 신원을 확인하고 인가는 접근 허용 여부를 확인한다.","두 가지 모두 올바르지 않다."]'::jsonb,'{"correct":2}'::jsonb),
    ('df90e3d4e332f6f7c43fa753c06467ba','["fetch join 사용","EntityGraph 사용","N+1 쿼리에 대한 최적화 전략 설정","EAGER 로딩을 모든 관계에 적용한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('879c9764cb3865c1218f4ae8f7d9d361','["application.yml 파일에 csrf.enabled=false로 설정한다.","@EnableWebSecurity 설정 클래스 내에서 HttpSecurity를 통해 csrf().disable()을 호출한다.","spring.security.csrf.enable=true로 프로퍼티 파일에 활성화한다.","스프링 시큐리티 필터 체인에서 CSRF 토큰 검증 필터를 제거해야 한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('b64e9ad4aba6be94fca9da642c935131','["CacheManager","RedisTemplate","JedisClient","CachingConfigurer"]'::jsonb,'{"correct":1}'::jsonb),
    ('0f8bfbe66ab5ef3c75c1883e92eefc82','["비동기 메서드는 항상 새로운 쓰레드를 생성하여 실행됩니다.","스프링 컨텍스트가 모든 비동기 메서드 호출에 대해 결과를 기다리는 동안 블로킹합니다.","비동기 메서드에서 발생한 예외는 자동으로 처리되지 않습니다.","비동기 메서드의 결과를 받아오는 방법은 반드시 Future 객체를 사용해야 합니다."]'::jsonb,'{"correct":3}'::jsonb),
    ('9cecf8b55f65e40d6110c77ddc0fb425','["Consumer Group은 메시지를 처리하는 여러 컨슈머를 모아 하나의 단위로 취급하며, 각 파티션이 특정 컨슈머에게 고유하게 할당된다.","Consumer Group은 동일한 주제에 대한 모든 메시지를 읽기 위해 독립적으로 작동하는 컨슈머들의 집합이며, 각 파티션은 임의로 선택된 컨슈머에게 할당된다.","Consumer Group은 Kafka 클러스터 내에서 중복된 메시지 처리를 방지하기 위한 식별자로서 동작하며, 모든 파티션이 단일 Consumer Group에 속한다.","각 파티션은 특정 소비그룹의 컨슈머에게 고유하게 할당되며, 리밸런싱 시점에서 자동적으로 파티션이 재할당된다."]'::jsonb,'{"correct":3}'::jsonb),
    ('c02fd9c056279125794c0948bbfac604','["Producer","MessagePublisher","KafkaProducer","TopicPublisher"]'::jsonb,'{"correct":2}'::jsonb),
    ('b06196fe645ffc1dd5285879d5fcbcb0','["application.yml 파일 경로가 잘못되었다.","프로파일 이름이 설정되지 않았다.","application.yml 파일 내부에서 프로필 이름이 지정되어 있지 않다.","스프링 부트 애플리케이션이 로드할 때 프로필을 활성화하지 못한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('d442918ec99e9699fbd34f88b22ee66f','["fetch join 사용","Entity Graph 사용","Lazy Loading 사용","Eager Loading 사용"]'::jsonb,'{"correct":0}'::jsonb),
    ('cbd576203454ab27efe611fa34b15237','["org.springframework.kafka.core","org.apache.kafka.clients.consumer","org.springframework.kafka.listener","org.apache.kafka.connect.runtime"]'::jsonb,'{"correct":0}'::jsonb),
    ('519ca1e8c0a6136834ac141e2abb7295','["주문 상태 업데이트 로직이 성공적으로 실행됩니다.","read-only 트랜잭션에서는 데이터 변경이 불가능하여 예외가 발생합니다.","트랜잭션이 읽기 전용으로 설정되어도 데이터는 변경될 수 있습니다.","DB 연결이 되지 않습니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('f502888f3a21a6dc306fd46b9140e49c','["데이터를 성공적으로 캐싱하고 요청 시 복잡성을 줄입니다.","캐시에 저장된 데이터가 업데이트 되지 않습니다.","findUserById 메서드는 캐시에 저장된 데이터만 반환합니다.","이 코드는 퍼포먼스 저하를 일으킵니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('8b1133987f68736ba861470dc165cdfe','["동작은 정상적으로 이루어지며 두 계좌의 잔액이 변경됩니다.","fromAccount에서 금액을 빼고 toAccount에 추가하려고 시도했지만 비정상적인 트랜잭션으로 실패합니다.","transferMoney 메소드가 실행될 때 발생하는 예외는 롤백하지 않습니다.","위 코드에서는 @Transactional 어노테이션이 적용되지 않았습니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('7271b2d39102ccce924f37e63f8c23b2','["실행 중 예외가 발생한다.","정상적으로 데이터베이스에 엔티티가 저장된다.","데이터베이스 연결이 열리지만 아무 작업도 수행되지 않는다.","엔티티는 영속성 컨텍스트에만 들어간다."]'::jsonb,'{"correct":0}'::jsonb),
    ('6558a7a39c36b82878754e46353ab656','["메서드를 동기화 처리한다.","스프링 빈 프록시를 직접 호출하여 트랜잭션을 생성한다.","인터페이스 메서드를 통해 메서드를 호출한다.","@Transactional 어노테이션을 사용하지 않고 별도로 트랜잭션을 시작한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('124e4d000eeb3bf192eb67359e49978a','["트랜잭션을 시작하지 않고 데이터를 읽는다.","데이터를 읽지만 변경사항을 커밋할 수 없다.","데이터베이스에 대한 모든 쓰기 작업을 금지한다.","데이터베이스에서 읽은 내용만 메모리에 저장한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('4d2eecfd3f5ac32f2d080d151380694c','["데이터베이스에서 엔티티를 삭제한다.","엔티티의 삭제 요청만 저장하여 커밋 시 처리된다.","readOnly 설정으로 인해 예외가 발생한다.","메서드 호출은 무시되고 아무런 동작도 수행하지 않는다."]'::jsonb,'{"correct":2}'::jsonb),
    ('dbb2e602b0c7d2b9c917500584b391ee','["트랜잭션이 유지되고 엔티티가 저장된다.","트랜잭션은 롤백되지만 엔티티는 저장되지 않는다.","엔티티는 저장되지만 트랜잭션이 예외로 인해 롤백된다.","예외 처리 없이 프로그램이 종료된다."]'::jsonb,'{"correct":2}'::jsonb),
    ('bdd57facf82271bcdd18c8fe7b15b5d1','["데이터베이스에 엔티티를 저장한다.","엔티티의 변경사항을 커밋하지 않는다.","readOnly 설정으로 인해 예외가 발생한다.","데이터베이스에서 읽은 내용만 메모리에 저장한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('5b8505121fcdddf3c7e4b3402ab07633','["props가 변경될 때마다 새로운 객체를 반환해야 합니다.","키 값을 고유하게 유지해야 합니다.","키 값은 변경되면 안 됩니다.","컴포넌트는 무효화되어야 합니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('c37aba9c10b64bd53f3023e7dabb970f','["전역 상태 관리가 필요할 때","특정 컴포넌트 범위 내의 상태만 관리할 때","상태 변화에 따른 렌더링 최적화를 원할 때","컴포넌트 트리를 쉽게 테스트하고 싶을 때"]'::jsonb,'{"correct":0}'::jsonb),
    ('634bd96db3ea552b159b38b79538d192','["API 요청 후 상태를 업데이트합니다.","API 요청 전 상태를 업데이트합니다.","API 요청과 동시에 상태를 업데이트합니다.","API 요청 대신 콜백을 사용하여 상태를 업데이트합니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('dc6b29f5f33658b6c1ca4fefb28d070c','["const [count, setCount] = useState(0);","const [count, setCount] = useState(() => 1 + Math.random());","const [count, setCount] = useState(() => (Math.random() < 0.5) ? 0 : 1);","const [count, setCount] = useState(Math.floor(Math.random() * 2));"]'::jsonb,'{"correct":0}'::jsonb),
    ('72c0e99427696324dd67e00c25ee99bd','["props는 반드시 객체 형태여야 한다.","props는 부모 컴포넌트에서 자식 컴포넌트로 전달된다.","props는 동적으로 변경될 수 있다.","props는 항상 불변해야 한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('6ff09b9aa4ba861e7ba489779c598e72','["더 빠른 업데이트","더 쉬운 상태 관리","덜 자주 리렌더링","더 적은 메모리 사용"]'::jsonb,'{"correct":1}'::jsonb),
    ('fb216352b6db0f371c7dab6ee23fe0e3','["useReducer()","useState()","useCallback()","useMemo()"]'::jsonb,'{"correct":1}'::jsonb),
    ('6bf8622cebedcfdb07a22229a9610943','["useEffect()를 이용한 배치 업데이트","useState()를 두 번 호출","setState() 메서드를 직접 사용","React.memo()와 shouldComponentUpdate() 메소드를 함께 사용"]'::jsonb,'{"correct":0}'::jsonb),
    ('e34e3d7781e318d8f8c6f903791faa43','["무한 루프 생성","배치 업데이트 발생","상태 누락","렌더링 성능 저하"]'::jsonb,'{"correct":0}'::jsonb),
    ('94102fa26daf5cd48fe15f7d04cdb220','["렌더링 직후","렌더링 중","렌더링 전","렌더링 후"]'::jsonb,'{"correct":0}'::jsonb),
    ('b37726feaf4053fe2e58255d791a0c14','["function MyComponent() { const [value, setValue] = useState(''''); return <input value={value} onChange={(e) => setValue(e.target.value)} />; }","function MyComponent() { let value = ''''; return <input value={value} onChange={(e) => value = e.target.value} />; }","function MyComponent() { const value = ''''; return <input value={value} onChange={(e) => value = e.target.value} />; }","function MyComponent() { let [value, setValue] = useState(''''); return <input value={value} onChange={(e) => value = e.target.value} />; }"]'::jsonb,'{"correct":0}'::jsonb),
    ('8bb4402d508d09979ae13c848a591028','["배치 업데이트가 일어나지 않음","무한 루프로 인해 성능 저하","렌더링 순서가 변경됨","상태 업데이트가 즉시 이루어짐"]'::jsonb,'{"correct":1}'::jsonb),
    ('2430fc0b8b29307bdffa3aec408309fa','["비동기 콜백을 사용하지 않음","렌더링 후 즉시 업데이트 상태","이벤트 핸들러를 직접 호출","비동기 동작을 처리하는 방법 변경"]'::jsonb,'{"correct":1}'::jsonb),
    ...
    ('9487a4b533d3cf90137b0928f9013811','["onChange 이벤트 핸들러에서 useState 훅 사용","value 속성을 props로 전달","defaultValue 속성 설정","useContext 훅 사용"]'::jsonb,'{"correct":0}'::jsonb),
    ('78ba94af7ea77a97157bdcc0afe99d96','["props로 전달","setState 메서드를 직접 호출","useState 훅을 사용","클래스의 this.state를 변경"]'::jsonb,'{"correct":2}'::jsonb),
    ('e48673c86aa8ec913d7b4c29f19d893e','["렌더링 성능 최적화","동일한 렌더링 결과 보장","DOM 업데이트 효율성 향상","리액트 내부에서의 상태 관리"]'::jsonb,'{"correct":2}'::jsonb),
    ('3f3a43cb10cc75c613754147dbd3094e','["useNavigate({ replace: true })","useNavigate({ state: { data } })","useNavigate({ preventDefault: true })","useNavigate({ navigateOptions: { preventBrowserNavigation: true } })"]'::jsonb,'{"correct":1}'::jsonb),
    ('5a402f931d6b80c373a3abd1b7041dec','["function MyComponent() { useEffect(() => {}, []); return <div>Content</div>; }","function MyComponent() { useState(0); useEffect(() => {}, []); return <div>Content</div>; }","function MyComponent() { const [count, setCount] = useState(0); useEffect(() => {}, []); return <div>Content</div>; }","function MyComponent() { useEffect(() => {}, []); useState(0); return <div>Content</div>; }"]'::jsonb,'{"correct":2}'::jsonb),
    ('7867481175fc7a837eda042f408ddbaa','["렌더링 성능이 저하되는 모든 컴포넌트에 적용해야 함","props 변경 시 컴포넌트를 재렌더링하지 않도록 하기 위해 사용","컴포넌트의 상태 관리에 필요한 최소한의 정보만 보존하도록 사용","상태 변화 없이도 컴포넌트가 렌더링되는 경우에만 사용"]'::jsonb,'{"correct":1}'::jsonb),
    ('77c9be5529ed9af935295b249a34fc00','["상태 변화에도 불구하고 컴포넌트는 무한 재렌더링을 수행하지 않음","컴포넌트는 상태 변경 이벤트를 감지하지 못하고 업데이트되지 않음","의존성 배열에 상태가 포함되어야 하므로 컴포넌트가 오류 발생","상태 변화에도 불구하고 useEffect 내부에서만 상태 업데이트가 적용됩니다"]'::jsonb,'{"correct":1}'::jsonb),
    ('3cb68d7f38e6dda83967b62bf603da41','["useState에 새로운 값으로 설정","useEffect 내부에서 setState 호출","initialState 매개변수로 전달","props 통해 전달"]'::jsonb,'{"correct":0}'::jsonb),
    ('b41a5412e3eb813cc684ec41dd71aed4','["두 개의 콘솔 로그가 모두 0으로 출력된다.","두 개의 콘솔 로그가 모두 최신 상태인 count로 출력된다.","setTimeout 내부에서 첫 번째 콘솔 로그는 이전 상태, 두 번째 콘솔 로그는 업데이트된 상태를 출력한다.","버튼 클릭 시 컴포넌트가 재렌더링되지 않는다."]'::jsonb,'{"correct":2}'::jsonb),
    ('8d608e299334ea3c8f90b6f87609264d','["useEffect 의 의존성 배열에 count를 추가해야 한다.","setInterval 대신 setTimeout을 사용하면 무한 루프가 해결된다.","무한 루프는 생성되지 않는다. setCount의 호출은 즉시 리렌더를 일으키지 않기 때문에 useEffect가 무한 재호출되는 문제가 없다.","useEffect 의 의존성 배열에서 window 객체를 추가해야 한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('11f089db53a0b6746f7494868b9d00c5','["클래스 컴포넌트에서 setState를 사용할 때 this.setState가 아닌 다른 메서드로 상태 업데이트를 시도함.","setState 내부에서 this.state.count를 직접 참조하여 stale closure 문제가 발생합니다.","클릭 이벤트 핸들러 handleClick이 정의되지 않았습니다.","클래스 컴포넌트는 함수형 컴포넌트보다 성능이 떨어집니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('f4bd54a2bae2102d3117bb088b535164','["count가 무한히 증가하지만, 컴포넌트 언마운트 시 정상적으로 인터벌을 제거합니다.","count가 1씩 증가하지만, 의존성 배열에 count가 없기 때문에 stale closure 문제를 일으킵니다.","count가 정확하게 1씩 증가하며 의존성 배열은 정상적으로 작동합니다.","count가 초기화되지 않고 무한히 증가하는 버그가 발생합니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('f1b49d2c1cf04b902d5f85cc277cc883','["클래스 컴포넌트에서 상태 업데이트 메서드를 직접 사용할 수 없다.","this.setState를 호출하면 클래스 내부의 다른 함수에서도 이 상태값을 참조해야 한다.","이 코드는 정상적으로 동작한다.","클릭 시 클래스 인스턴스가 새롭게 생성된다."]'::jsonb,'{"correct":0}'::jsonb),
    ('79a70e5676e2fe47451358893cc9911e','["버튼 클릭 시 count 상태가 1씩 증가합니다.","버튼 클릭 시 count 상태가 계속해서 무한히 증가합니다.","버튼 클릭 시 아무런 동작이 일어나지 않습니다.","버튼 클릭 시 컴포넌트가 언마운트됩니다."]'::jsonb,'{"correct":2}'::jsonb),
    ('6cfe33ea40b863bd770352aea6e78f75','["이 코드는 정상적으로 동작한다.","handleChange 메서드에서 this.setState를 호출하면 무한 루프가 발생할 수 있다.","클래스 컴포넌트에서는 value 속성을 사용하지 않고, onChange 이벤트 핸들러만 사용해야 한다.","인풋 필드의 값이 변경될 때마다 컴포넌트가 재렌더링되지 않는다."]'::jsonb,'{"correct":1}'::jsonb),
    ('59c14f9908e4c36567a1080a7b4a271a','["렌더링할 때마다 새로운 엘리먼트 생성","렌더링할 때마다 기존 엘리먼트 재사용","항상 동일한 엘리먼트를 참조","변경된 props로 인해 업데이트"]'::jsonb,'{"correct":1}'::jsonb),
    ('6b522e308f5aee3ab56b777e175cbfbb','["React.memo는 props 변경에 따라 항상 컴포넌트를 렌더링한다.","React.memo는 이벤트 핸들러의 변경사항을 감지하지 못한다.","props 값이 바뀌면 React.memo가 무시되고 컴포넌트가 계속 렌더링된다.","setInterval로 props 값이 자주 변경되므로 React.memo가 동작하지 않는다."]'::jsonb,'{"correct":3}'::jsonb),
    ('cc8294c863b177a3c524d627e1d75d95','["배열에 ''item1''이 정상적으로 추가됩니다.","배열은 변하지 않고 아무 일도 발생하지 않습니다.","배열은 변하지만 컴포넌트는 리렌더되지 않습니다.","컴포넌트가 무한 루프로 리렌더됩니다."]'::jsonb,'{"correct":3}'::jsonb),
    ('890d9b889569b1e332c8ac5219527464','["React.memo는 props 변경에 따라 항상 컴포넌트를 렌더링한다.","React.memo는 이벤트 핸들러의 변경사항을 감지하지 못한다.","props 값이 바뀌면 React.memo가 무시되고 컴포넌트가 계속 렌더링된다.","setInterval로 props 값이 자주 변경되므로 React.memo가 동작하지 않는다."]'::jsonb,'{"correct":3}'::jsonb),
    ('e9c212dc157daa7e6d684b0569607c37','["렌더링 후 즉시 실행","렌더링 전에 실행되며 블록킹 가능","렌더링 중 단계에서 무시됨","렌더링 시점과 독립적으로 실행"]'::jsonb,'{"correct":1}'::jsonb),
    ('5b041101a841b64cedbda58a4f4e29af','["setTimeout 내부에서 setState를 사용하는 것은 문제 없습니다.","setState 메서드는 setTimeout 내부에서는 동작하지 않습니다.","setTimeout 내부에서 this.state.count를 참조하면 stale closure 문제가 발생합니다.","클래스 컴포넌트에서는 setTimeout을 사용할 수 없습니다."]'::jsonb,'{"correct":2}'::jsonb),
    ('bffd1d3e2e0a0e72ae56fe0b94b45b29','["useLayoutEffect는 필요하지 않다.","렌더링 후 DOM 업데이트 전에 작업을 수행하기 위해 useLayoutEffect가 사용되었다.","렌더링 중인 컴포넌트의 상태를 변경해야 할 때, useLayoutEffect가 사용된다.","useLayoutEffect와 useEffect는 같은 역할을 한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('85433f23873e6df5c3ba065f50492206','["setTimeout 내부에서 this.setState를 사용하는 것은 문제가 없습니다.","setState 메서드는 setTimeout 내부에서는 동작하지 않습니다.","setTimeout 내부에서 this.state.count를 참조하면 stale closure 문제가 발생합니다.","클래스 컴포넌트에서는 setTimeout을 사용할 수 없습니다."]'::jsonb,'{"correct":2}'::jsonb),
    ('df25e80c300907819cb009030c284d7d','["setTimeout 내부에서 this.setState를 사용하여 상태 업데이트 시 stale closure 문제가 발생합니다.","클래스 컴포넌트에서는 setTimeout을 사용할 수 없습니다.","setState 메서드가 정의되지 않았습니다.","카운터 값이 증가하지 않습니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('11ca8164598e0fdf063c51a922484ec7','["useContext는 컴포넌트 트리에서 가장 가까운 컨텍스트 값을 가져온다.","useContext 훅은 직접 제공된 value값을 무시하고, 최상위 Provider의 값만 사용한다.","useEffect 내부에서 useContext 훅이 호출되면, 이 컴포넌트는 렌더링되지 않는다.","ThemeContext.Provider가 없는 상황에서도 useContext가 정상적으로 작동한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('7ff8b4154fd67436050630e7cdb3fb67','["제약은 위로, 크기는 아래로 전파된다.","크기는 위로, 제약은 아래로 전파된다.","제약과 크기 모두 위로 전파된다.","제약과 크기 모두 아래로 전파된다."]'::jsonb,'{"correct":0}'::jsonb),
    ('82b3a1c3af14f8055ac159d87f0fa627','["await 키워드 없이 Future 반환 함수 호출하기","Future 객체가 반환되는 함수 안에서 await 키워드를 사용하여 다른 Future 기다리기","비동기 함수에서 return 키워드만으로 값을 반환하기","Future.delayed로 지연된 작업을 실행하지 않고 바로 반환"]'::jsonb,'{"correct":1}'::jsonb),
    ('1f3d948bd49b69275565146167e5a51c','["BuildContext 변경 시 rebuild","Provider 상태가 변경될 때 rebuild","위젯 트리 전체 rebuild","특정 위젯만 rebuild"]'::jsonb,'{"correct":3}'::jsonb),
    ('b2a130028e626b6f67ad815b5ac957da','["FutureBuilder 위젯은 Future 객체를 반환하는 함수를 받아들입니다.","StreamBuilder 위젯은 Stream 객체를 반환하지 않습니다.","async/await 키워드는 동기적인 코드로 실행됩니다.","dispose 메서드는 StatefulWidget에서 항상 호출되지 않습니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('675613c097b1b42cf2079be5a1bdd481','["initState는 위젯이 생성될 때 호출되고, dispose는 위젯이 더 이상 사용되지 않을 때 호출된다.","initState는 위젯이 화면에 보일 때 호출되며, dispose는 위젯이 더 이상 보여지지 않을 때 호출된다.","initState는 위젯의 상태를 초기화하고, dispose는 리소스를 해제한다.","initState와 dispose 모두 위젯이 생성될 때 호출되며, 상태 관리를 위한 초기 설정을 수행한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('80eceb0768378485b02bfd78ebeec72c','["children 속성에 Expanded 객체 추가하기","mainAxisAlignment 속성에 MainAxisAlignment.spaceAround 설정하기","crossAxisAlignment 속성에 CrossAxisAlignment.start 설정하기","flex 속성 값을 변경하여 자식 위젯 사이 간격 조절"]'::jsonb,'{"correct":0}'::jsonb),
    ('3d98b0371f5359b70219bc700802b373','["상태를 변경해야 하는 경우 StatelessWidget을 사용한다.","상태를 변경하지 않는 경우 StatefulWidget을 사용한다.","상태를 변경해야 하는 경우 StatefulWidget을 사용한다.","상태를 변경하지 않는 경우 StatelessWidget을 사용한다."]'::jsonb,'{"correct":3}'::jsonb),
    ('fa951e4c67df5264ccd0192174782c8c','["Stack 위젯 내부에 Positioned 위젯을 사용하고, top과 left 속성을 0으로 설정한다.","Stack 위젯 내부에 Positioned 위젯을 사용하고, top와 left 속성을 화면 크기의 절반으로 설정한다.","Row 위젯과 Column 위젯을 이용하여 아이콘을 중앙 위치로 배치한다.","Expanded 위젯을 사용하여 아이콘을 화면 중앙에 위치시킨다."]'::jsonb,'{"correct":1}'::jsonb),
    ('d7837b72b3c5e37c77982f8c25d0395b','["항목의 개수를 미리 계산한다.","건너뛰기(index)로 특정 항목만 빌드한다.","항목을 모두 미리 생성해 놓는다.","UI가 필요할 때마다 즉시 생성한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('0442352ed97a524c7f12793342521767','["Navigator의 push 메서드는 현재 스택에 있는 모든 위젯을 제거한다.","Navigator의 pop 메서드는 현재 활성화된 스택 위젯을 화면에서 제거한다.","Navigator의 push 메서드는 새로운 페이지를 스택에 추가하고, 이전 페이지로 돌아가는 버튼을 제공한다.","Navigator의 pop 메서드는 가장 상위 레벨 페이지로 즉시 이동한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('6041ffc4214fd27e38f32c4e58d8d0f9','["InheritedWidget의 서브클래스를 제공한다.","상태 변경 시 build 메서드를 호출한다.","데이터 흐름을 관리하는 역할을 한다.","UI와 비즈니스 로직 사이에 통로를 만든다."]'::jsonb,'{"correct":2}'::jsonb),
    ('ceb69687398ddf8d580ae7bd408cc34f','["Container 위젯의 크기","Stack 위젯의 크기","화면 전체의 크기","자신의 부모 위젯의 크기"]'::jsonb,'{"correct":1}'::jsonb),
    ('8d43d15277d610538bbd91dace98efa7','["isolate는 Dart VM에서 별도의 실행 컨텍스트를 제공하며, 메인 isolate와 통신하려면 MessageChannel을 사용한다.","이벤트 루프는 비동기 작업이 완료될 때마다 이벤트를 처리하지만, 동기적으로 코드가 실행되는 동안에도 계속 작동한다.","isolate는 프로그램의 전체 생명주기를 관리하며, 메인 isolate에서 생성된 모든 isolate는 자동으로 종료된다.","이벤트 루프는 비동기 작업을 큐에 넣어 처리하지만, 이벤트가 발생하지 않으면 이벤트 루프도 중단된다."]'::jsonb,'{"correct":0}'::jsonb),
    ('f99f6ce8368535c872b0d091f651c5d8','["Platform Channel","PermissionHandler 라이브러리","InAppWebViewController","Firebase Cloud Messaging"]'::jsonb,'{"correct":3}'::jsonb),
    ('6f1be800378bae842d7aa3409230d4ff','["Provider 패키지를 사용하여 상태를 공유합니다.","각 위젯 내부에 상태를 직접 관리합니다.","InheritedWidget을 사용하여 위젯 트리 전체에 상태를 전달합니다.","Riverpod 패키지를 사용하여 컴포지션 레이어에서 상태를 관리합니다."]'::jsonb,'{"correct":3}'::jsonb),
    ('5c652a2df2dab9d5759eb2d9ec6c3ef3','["하나의 경로(String)만 필요합니다.","이전 화면으로 돌아가는 방법과 함께 두 개 이상의 경로가 필요합니다.","이동할 페이지에서 전달할 인자와 함께 하나의 경로가 필요합니다.","새로운 라우트를 생성하기 위한 클래스 정의가 필요합니다."]'::jsonb,'{"correct":2}'::jsonb),
    ('8159b7f03e3ece53dab26cb1990f5060','["하위 자식들의 요구(constraints)를 기반으로 결정","상위 부모의 크기를 그대로 상속받음","하위 자식의 크기와 무관하게 고정된 값","상위 부모의 크기에 비례하여 확장"]'::jsonb,'{"correct":0}'::jsonb),
    ('346e484401e378c5e3e4ae5a0219878b','["closeSocketConnection(); 뒤에 super.dispose();를 추가한다.","super.dispose(); 뒤에 closeSocketConnection();를 추가한다.","코드에서 fetchUserData()를 제거한다.","코드를 수정하지 않고도 누수가 발생하지 않는다."]'::jsonb,'{"correct":1}'::jsonb),
    ('c5041f52f4f1d5c37cc2e2c90a7c4ff4','["didUpdateWidget 메서드에서 MyInheritedWidget의 rebuild 메소드 호출.","didUpdateWidget 메서드에서 MyInheritedWidget.of(context).notifyListeners();를 추가.","build 메서드에서 MyInheritedWidget.of(context).rebuild()를 호출.","didUpdateWidget 메서드에서 super.didUpdateWidget(oldWidget)을 먼저 호출하라."]'::jsonb,'{"correct":1}'::jsonb),
    ('3a9eaa5aba8e614aacf136d616b6b9bb','["Expanded 위젯","Flexible 위젯","Container 위젯","Positioned 위젯"]'::jsonb,'{"correct":3}'::jsonb),
    ('b2d9bfee64210736f55c94101ea4719a','["위젯 트리는 Stack 위젯의 자식으로 정확히 위치 지정된 이미지와 빨간색 컨테이너가 포함됩니다.","위젯 트리에서 이미지는 항상 화면 중앙에 위치하며, 컨테이너는 왼쪽 상단 50px, 100px에 위치합니다.","위젯 트리는 정확히 지정된 위치에 빨간색 컨테이너만을 포함합니다.","위젯 트리에서 이미지는 화면 전체를 차지하고 컨테이너는 왼쪽 상단 50px, 100px에 위치합니다."]'::jsonb,'{"correct":3}'::jsonb),
    ('92d8157d46619a0965997680da85a60a','["MyWidget은 상태가 변경될 수 있습니다.","MyWidget은 상수(const)로 정의되었기 때문에 불변합니다.","build 메서드는 항상 호출됩니다.","const 키워드를 사용하면 위젯이 재렌더링되지 않습니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('1cd91500fa3ca86fec2f731e3b9c3e50','["Container 위젯에 height 속성을 제거한다.","ListTile 위젯을 사용하여 itemBuilder 메소드를 수정한다.","ListView.builder에서 itemCount 대신 count를 사용한다.","itemBuilder 메서드에서 setState 호출을 제거한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('827dde99874e8dbd32def72b276598a9','["setState를 사용하면 Counter의 상태가 변경될 때마다 위젯 트리가 재구축됩니다.","setState를 사용하지 않으면 Counter의 상태가 변경되었을 때도 위젯 트리는 그대로 유지됩니다.","setState는 Counter의 상태를 변경하지만, build 메서드에서 반환되는 위젯은 변하지 않습니다.","setState를 사용하면 Counter의 상태가 변경될 때마다 Flutter 프레임워크에 이벤트를 전달합니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('a6c0bd2bab7043c8ff07be8b6340c6e0','["비동기 작업이 완료되면 ''Loading...''이라는 텍스트가 계속 표시됩니다.","비동기 작업 중 오류 발생 시 에러 메시지가 화면에 표시됩니다.","비동기 작업의 결과를 정상적으로 화면에 표시합니다.","비동기 작업이 완료되지 않으면 ''Awaiting connection...''이라는 텍스트만 표시됩니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('017070ae34102463d9bd761e769d4671','["Pod는 항상 동작하지 않을 것이다.","Pod는 여러 복제본이 필요하다.","Pod는 특정 네트워크 이름을 사용해야 한다.","Pod는 스펙에서 정의된 리소스를 지원하지 못한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('b4a855e1c25bfc2142a8d9df2d61df1a','["불필요한 명령을 줄인다.",".dockerignore 파일 사용","빌드 타임 캐시 활성화","Docker Hub 이용"]'::jsonb,'{"correct":0}'::jsonb),
    ('b9ca8dda37b6fbe5b2606269e15d8b31','["blue-green 배포","canary 배포","rolling 배포","full 배포"]'::jsonb,'{"correct":0}'::jsonb),
    ('2fcf749836371d7d868cf9c319e610f6','["로그","메트릭","알림","대시보드"]'::jsonb,'{"correct":1}'::jsonb),
    ('7084b2d20ebc92793e72f3d1123778e9','["서비스 중단 시간 발생","데이터 손실 위험","스케일링 이슈","노드 리소스 오버헤드"]'::jsonb,'{"correct":0}'::jsonb),
    ('f85dffe51562c83663834e00c806d064','["livenessProbe","readinessProbe","startupProbe","execProbe"]'::jsonb,'{"correct":0}'::jsonb),
    ('4357b55cade2c03c1252355c39526d7b','["ADD","COPY","RUN","SHELL"]'::jsonb,'{"correct":1}'::jsonb),
    ('5ad2eaf854ba47b1233c7f9a8132c382','["Service는 항상 동작하지 않을 것이다.","Service는 여러 복제본이 필요하다.","Service는 특정 네트워크 이름을 사용해야 한다.","Service는 스펙에서 정의된 리소스를 지원하지 못한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('f6a8cc055716ec8b0dcb13f9762f944b','["볼륨은 호스트와 연결, 바인드는 컨테이너 내부","볼륨은 컨테이너 내부, 바인드는 호스트","볼륨은 영속성 보장, 바인드는 일시적 데이터 저장","볼륨은 일시적 데이터, 바인드는 영속성"]'::jsonb,'{"correct":2}'::jsonb),
    ('7428d1e6ac5b80bf3b0da53f9bf72ac0','["Pod가 실행되면 즉시 응답을 반환해야 할 때.","Pod가 준비되어 서비스에 대응할 수 있을 때.","Pod가 오류로 인해 계속 재생성되는 경우.","Pod가 정상적으로 작동하며 메모리를 사용하는 경우."]'::jsonb,'{"correct":1}'::jsonb),
    ('3ac586e3f6e3ab3febf09bf8b81bebee','["Pod가 정상적으로 실행되었는지를 확인한다.","서비스 준비 상태를 확인하고 요청을 받아들일 수 있는지 여부를 결정한다.","초기화 시간이 긴 서비스에 대해 추가적인 타임아웃을 설정한다.","리소스 사용량을 모니터링하여 과부하를 감지한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('f86831a42b630d62ca9f24612f9c4c03','["GitOps는 배포 과정에서 실시간 동기화를 지원하지 않는다.","GitOps는 롤백을 지원하지 않는다.","GitOps는 보안을 충분히 고려하지 못한다.","GitOps는 다양한 CI/CD 도구와 호환되지 않는다."]'::jsonb,'{"correct":1}'::jsonb),
    ('32925f1cc430c15064088282b8367554','["실행 중인 컨테이너의 변경 사항을 영구적으로 저장해야 할 때.","컨테이너 이미지 크기를 줄이는 방법으로 사용할 때.","동일한 볼륨을 여러 컨테이너 간에 공유해야 할 때.","기존 파일 시스템에 대한 직접 접근 권한 필요시."]'::jsonb,'{"correct":0}'::jsonb),
    ('280f9e0e119d271eafc6a5a2d68ca050','["상태 체크 시점의 차이","서비스 가용성 확인과 노드 상태 확인","리소스 사용량 차이","네트워크 연결 상태 차이"]'::jsonb,'{"correct":1}'::jsonb),
    ('ddbad386c9fa2f1f300a8a8be5fdd081','["kubectl apply -f deployment.yaml","kubectl rollout status deployment/name","kubectl set image deployment/name=image:tag","kubectl rollout undo deployment/name"]'::jsonb,'{"correct":2}'::jsonb),
    ('f0ca854f6ab2e8090c288b169253e4d2','["기존 애플리케이션과 새 버전을 동시에 실행한다.","새로운 버전만 먼저 실행한 후 기존 버전을 삭제한다.","기존 애플리케이션을 완전히 중지한 다음 새로운 버전으로 대체한다.","신규 애플리케이션이 준비되면 트래픽을 빠르게 전환한다"]'::jsonb,'{"correct":3}'::jsonb),
    ('9e46986a6c8b01971a7b142a5bfac5af','["빌드 과정에서 캐시 활용이 제대로 이루어지지 않는다.","볼륨 마운트가 제대로 설정되어 있지 않다.","멀티스테이지 빌드를 지원하지 않는다.","Docker 이미지를 최적화할 수 없다."]'::jsonb,'{"correct":0}'::jsonb),
    ('906687f95bf83ad5dd6d6e5118af1901','[".dockerignore 파일을 수정하여 불필요한 파일을 제거한다","이미지 빌드 시마다 모든 레이어를 새로 만든다","Dockerfile에서 불필요한 명령어를 줄인다","빌드 후에 이미지를 압축한다"]'::jsonb,'{"correct":0}'::jsonb),
    ('7d1b693e2b56a6a0f01219e08c5822ae','["Blue-Green 배포는 새로운 버전의 서비스를 실행시키지 않고 기존 서비스만 업데이트한다.","Canary 배포는 전체 트래픽을 단번에 새 서비스로 이동시킨다.","Blue-Green 배포는 기존 서비스와 새 서비스가 동시에 동작하며 트래픽 분리를 허용하고, Canary 배포는 부분적인 트래픽만 새 서비스로 보내서 점진적으로 확장한다.","Canary 배포는 모든 트래픽을 단계별로 이동시키며 Blue-Green은 한 번에 전체를 업데이트한다."]'::jsonb,'{"correct":2}'::jsonb),
    ('3bb2064a819dcf59cb56dbfcda872a03','["이미지 레이어를 최소화하는 명령을 사용한다.","도커 이미지를 항상 새로운 빌드로 생성한다.","볼륨 마운트를 사용하여 실행 시점의 변경 사항을 반영한다.","Dockerignore 파일을 통해 불필요한 파일을 배제한다."]'::jsonb,'{"correct":3}'::jsonb),
    ('824423ae743913f475550e29d0e8ff52','["이미지를 실행합니다.","빌드 과정에서 변경된 파일만 새로 빌드합니다.","도커 이미지 레이어를 최적화합니다.","기존 이미지와 비교하여 새로운 이미지만 생성합니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('dd04f067524adcbf92f148628787a354','["이미지 레이어 최적화","Dockerfile에서 사용하지 않는 파일 제거","루트 권한으로 컨테이너 실행","도커 인스턴스 수 증가"]'::jsonb,'{"correct":0}'::jsonb),
    ('f65113ecf4e1ab26d0585961598c7a4e','["빠른 롤백 가능","서비스 중단 시간 최소화","데이터 동기화 문제 해결","스케일링 이슈 개선"]'::jsonb,'{"correct":0}'::jsonb),
    ('ce5e961b1bcce0247607dc4cc573dcef','["HPA는 CPU 사용률을 기준으로만 스케일링할 수 있다.","HPA는 메모리 사용률을 기준으로 스케일링할 수 없다.","HPA는 CPU와 메모리를 동시에 감지하지 못한다.","HPA 설정이 잘못되어 있어 실제로 동작하지 않을 것이다."]'::jsonb,'{"correct":3}'::jsonb),
    ('945f15ea351fa4a0228e21afdc0f88f1','["Blue-Green Deployment","Canary Release","Rolling Update","GitOps"]'::jsonb,'{"correct":0}'::jsonb),
    ('295f2eae45d00d265d254e21ab189299','["Counter: 증가만을 추적하는 측정값.","Gauge: 특정 시간에 대한 상태를 나타내는 값.","Histogram: 분산된 값을 그룹화하여 수집한다.","Summary: 분산된 값과 누적된 값을 동시에 제공한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('fbc9d661a00ad9bc3eaa458babdc2b8d','["Pod는 일회성 작업을 실행하며, Deployment는 지속적인 서비스를 제공한다.","Deployment는 Pods의 세트를 관리하고 스케일링하며, Pod는 단일 컨테이너 엔터티다.","Pod는 복수의 컨테이너를 포함할 수 있으며, Deployment는 단일 컨테이너만 가능하다.","Deployment는 상태를 유지하지 않으며, Pod는 항상 최신 상태로 유지된다."]'::jsonb,'{"correct":1}'::jsonb),
    ('0ab078a06a4a79e35f6527f97b7e290f','["Prometheus","Grafana","Elasticsearch","Kibana"]'::jsonb,'{"correct":0}'::jsonb),
    ('a522525cd7f10abc392a7c20a256996d','["Docker 이미지 빌드 시 실행할 기본 명령어를 지정한다.","Docker 컨테이너가 외부 네트워크에 접근하도록 설정한다.","Docker 컨테이너 내에서 필요한 패키지를 설치한다.","Docker 이미지의 작성을 위한 레이어를 구성한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('055237abf1008503f0d80a4f5e157883','["replica 개수 설정이 잘못되었습니다.","selector와 template의 라벨이 일치하지 않습니다.","containerPort가 정확하게 지정되지 않았습니다.","deployment에 대한 resource limit이 누락되었습니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('d2ac780a2082e6c7226c15e593b61a8e','["minReplicas와 maxReplicas 설정이 잘못되었습니다.","targetCPUUtilizationPercentage가 너무 낮습니다.","scaleTargetRef에 Deployment 이름을 잘못 지정했습니다.","HorizontalPodAutoscaler에 대한 metrics server가 설치되지 않았습니다."]'::jsonb,'{"correct":3}'::jsonb),
    ('020c2729c404cd49aec2401c2ef525e1','["Dockerfile는 모든 의존성을 설치한 후에 애플리케이션 소스 코드를 복사하므로 이미지 크기를 최적화한다.","Dockerfile은 먼저 모든 의존성을 설치하고 나서 애플리케이션 소스 코드를 복사하여 이미지 크기를 줄인다.","Dockerfile은 의존성 설치 전에 애플리케이션 소스 코드를 복사하므로 이미지 크기가 커진다.","Dockerfile는 이미지 빌드 과정에서 캐시 활용을 방해하는 명령어를 포함한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('ae2483a999135ffa62ae319c5b64514f','["Docker 서비스를 정의한다.","Docker Compose 프로젝트 설정을 구성한다.","Docker 컨테이너 네트워크를 생성한다.","Docker 이미지를 빌드한다."]'::jsonb,'{"correct":1}'::jsonb),
    ('92250b5164d0c8856d26e5d51fa11c46','["ReplicaSet은 정확히 3개의 Pod를 생성합니다.","ReplicaSet은 ''another-app'' 라벨을 가진 Pod 3개를 생성하려고 시도하지만 실패합니다.","ReplicaSet은 ''example-app'' 라벨을 가진 Pod 3개를 생성합니다.","ReplicaSet은 정확히 아무것도 생성하지 않습니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('04c5ef363ceb7cd1c4d259d852df6209','["이미지 크기를 줄이는 데 효과적이다.","도커 이미지를 최대한 빠르게 빌드한다.","코드를 실행할 때마다 모든 노드 모듈을 다시 설치한다.","도커 캐시가 무효화되어 각 단계에서 모든 파일이 새로 다운로드된다."]'::jsonb,'{"correct":1}'::jsonb),
    ('fdbd5ff9f94be230aa52e3d91c44e906','["Pod가 생성되면 즉시 시작됩니다.","readinessProbe가 성공하면 시작됩니다.","initialDelaySeconds 이후에 시작됩니다.","periodSeconds마다 시작합니다."]'::jsonb,'{"correct":2}'::jsonb),
    ('bec70924d0a90a0f2abedc2b4e1b955f','["Dockerfile는 ''npm start'' 명령으로 애플리케이션을 시작합니다.","Dockerfile는 ''node server.js'' 명령으로 애플리케이션을 시작합니다.","Dockerfile는 ''npm run build'' 명령으로 애플리케이션을 빌드한 후 실행하지 않습니다.","Dockerfile는 이미지를 생성할 때 ''npm install''과 ''npm run build''를 모두 수행합니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('71144bff57968c429cbe6994beb43ad6','["Secret은 암호화되어 저장되므로 안전하다.","Secret이 정상적으로 생성되지만, type 설정이 잘못되어 암호화되지 않는다.","Secret 데이터는 base64로 인코딩되어 있어 애플리케이션에서 직접 사용할 수 없다.","Secret은 정상적으로 생성되며, 애플리케이션이 이를 참조하여 환경 변수로 설정한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('204453b813a5376fd8bf634c0c03db6a','["토큰의 유효기간 설정","토큰 내 정보의 암호화","리프레시 토큰 회전","CORS 정책 설정"]'::jsonb,'{"correct":1}'::jsonb),
    ('0fa9fd839d28367dd36c985b00c432b3','["HTTP 상태 코드를 사용하여 세션을 관리합니다.","세션 쿠키를 클라이언트에 저장하고 이를 서버에서 확인합니다.","JWT 토큰을 사용하여 클라이언트와 서버 간의 자격 증명을 전달합니다.","클라이언트 측 세션 ID를 사용하여 서버와 통신합니다."]'::jsonb,'{"correct":2}'::jsonb),
    ('e51e4d9c079a34eccb65e1bbb7b7ab9e','["데이터의 일관성이 중요한 경우","속도와 가용성 요구사항이 높은 경우","복잡한 계층 구조가 필요한 경우","다중 연결 관계가 있는 데이터 모델링 시"]'::jsonb,'{"correct":1}'::jsonb),
    ('17538fe8e4233834e67eae3f514b8528','["UNIQUE 인덱스","CLUSTERED 인덱스","NONCLUSTERED 인덱스","FULLTEXT 인덱스"]'::jsonb,'{"correct":2}'::jsonb),
    ('7ee7cef95cca225d5bd32c874da86874','["200 OK","401 Unauthorized","500 Internal Server Error","408 Request Timeout"]'::jsonb,'{"correct":3}'::jsonb),
    ('76728b6570d2b820b9a958e946ed5039','["세션 기반 인증","HTTP 헤더를 통한 JWT 전송","쿼리 스트링을 통한 JWT 전송","클라이언트 쿠키 저장"]'::jsonb,'{"correct":1}'::jsonb),
    ('b7d43fd8aa5831ae29f5b7f31cd72734','["세션 기반 인증 사용하기","JWT 토큰을 클라이언트에 저장하지 않고 서버만 사용하기","토큰 만료 시간이 짧은 JWT를 사용하여 자주 새로 고침하도록 하기","프론트엔드에서 암호를 보관하고 직접 로그인 요청 보내기"]'::jsonb,'{"correct":2}'::jsonb),
    ('e4ceeebdf04eec98b7d2af30f35a4ad0','["클라이언트 측 캐싱","서버 사이드 캐싱","데이터베이스 인덱스 사용","모든 요청에 대한 로직 단순화"]'::jsonb,'{"correct":1}'::jsonb),
    ('d9be803797752baf78dec7de89cfde1b','["Database Session Store","Cookie-based Session Store","Memory-based Session Store","File System Session Store"]'::jsonb,'{"correct":1}'::jsonb),
    ('8b770428d2ac952f1c151cccb6621742','["서브도메인 기반 (v1.example.com)","쿼리 파라미터 기반 (/api/v1/resource)","헤더 기반 (X-API-Version)","path 인자 기반 (/v1/api/resource)"]'::jsonb,'{"correct":2}'::jsonb),
    ('3a204f71cb9601c0686f81e0230e4476','["새로 로그인하라는 메시지를 띄운다","프론트에서 JWT 리프레시 토큰을 사용하여 새 액세스 토큰을 발급한다","백엔드에 요청해 세션 정보를 재생성한다","현재 페이지는 유지하고 다른 모든 페이지에는 로그인 화면으로 이동한다"]'::jsonb,'{"correct":1}'::jsonb),
    ('22bcc9a03c96bd65a9e63f1ba03e2140','["200 OK","302 Found","401 Unauthorized","500 Internal Server Error"]'::jsonb,'{"correct":0}'::jsonb),
    ('40783d95d05416b528702256cfa43e99','["API 호출 시 오류 처리를 하지 않는다.","response 객체를 json으로 변환하지 않는다.","fetch 메서드는 GET 요청을 지원하지 않는다.","API 경로가 잘못되어 응답을 받지 못한다."]'::jsonb,'{"correct":0}'::jsonb),
    ('aca4f176cc9899d0b7bb5d63687e4466','["트랜잭션 안에서 사용자 정보를 업데이트합니다.","사용자의 모든 데이터를 삭제합니다.","트랜잭션이 실패할 경우 롤백됩니다.","HTTP 요청을 처리합니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('a658c72a3bc16772c5de86dee1199f03','["쿼리가 비정규화된 데이터베이스에 적합하지 않다.","email 필드와 status 필드를 인덱싱해야 한다.","users 테이블에서 모든 컬럼을 가져오는 것이 비효율적이다.","WHERE 절에서 email만 검색하는 것이 더 효율적이다."]'::jsonb,'{"correct":1}'::jsonb),
    ('bdb7fe1e8545f22f386654d32446899b','["API는 GET 요청에 대해 200 OK 상태 코드와 데이터를 반환합니다.","API는 POST 요청에 대해 405 Method Not Allowed 상태 코드를 반환합니다.","API는 GET 요청에 대해 500 Internal Server Error 상태 코드를 반환합니다.","API는 모든 사용자 정보를 DB에서 가져오지만 클라이언트에게 아무 것도 전송하지 않습니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('6ce3687e801d7b4bff26ef6e4ab6313e','["트랜잭션이 예외 발생 시 롤백되지 않는다.","메서드명에서 saveUser 대신 save를 사용해야 한다.","userRepository에 대한 의존성 주입이 누락되었다.","@Transactional 어노테이션을 사용했지만, 예외가 발생해도 항상 롤백된다."]'::jsonb,'{"correct":0}'::jsonb),
    ('230f42fbb04af38d1c59541dce668e96','["1. 배포 환경에서 데이터베이스 연결 상태를 확인하지 않습니다.","2. 항상 ''DOWN'' 상태를 반환합니다.","3. 헬스 체크 결과가 JSON 형식으로 반환됩니다.","4. ''database''와 같은 세부 정보는 없을 수 있습니다."]'::jsonb,'{"correct":0}'::jsonb),
    ('4689c468883a72b0ddaa33f386dc61fc','["쿼리는 인증된 사용자에게 로그인 세션을 생성합니다.","쿼리는 데이터베이스에서 사용자의 암호 해시를 검색합니다.","쿼리는 사용자가 입력한 패스워드와 일치하는지 확인합니다.","쿼리는 비밀번호 필드에 저장된 원문 암호를 반환합니다."]'::jsonb,'{"correct":1}'::jsonb),
    ('137196cf482c273634eca047895ae5d2','["@GetMapping(\"/profile\")\npublic UserDTO getUserProfile(@RequestParam String userId) {\n    User user = userRepository.findByUserId(userId);\n    if (user == null) {\n        throw new ResourceNotFoundException();\n    }\n    return modelMapper.map(user, UserDTO.class);\n}","@GetMapping(\"/profile\")\npublic UserDTO getUserProfile(@RequestParam String userId) throws UserRepositoryException {\n    try {\n        User user = userRepository.findByUserId(userId);\n        if (user == null) {\n            throw new ResourceNotFoundException();\n        }\n        return modelMapper.map(user, UserDTO.class);\n    } catch (UserRepositoryException e) {\n        throw new ResourceNotFoundException(e.getMessage());\n    }\n}","@GetMapping(\"/profile\")\npublic ResponseEntity<UserDTO> getUserProfile(@RequestParam String userId) throws UserRepositoryException {\n    try {\n        User user = userRepository.findByUserId(userId);\n        if (user == null) {\n            throw new ResourceNotFoundException();\n        }\n        return ResponseEntity.ok(modelMapper.map(user, UserDTO.class));\n    } catch (UserRepositoryException e) {\n        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);\n    }\n}","@GetMapping(\"/profile\")\npublic UserDTO getUserProfile(@RequestParam String userId) throws UserRepositoryException {\n    User user = userRepository.findByUserId(userId);\n    if (user == null) {\n        throw new ResourceNotFoundException();\n    }\n    return modelMapper.map(user, UserDTO.class);\n}"]'::jsonb,'{"correct":1}'::jsonb),
    ('4f68c031496ab3694f9e068014531ecf','["1. 토큰이 유효하지 않으면 오류를 처리합니다.","2. 모든 HTTP 응답 코드에 대해 성공으로 처리됩니다.","3. 요청한 데이터가 존재하지 않을 때도 항상 정상적으로 반환합니다.","4. 인증 헤더를 제대로 설정하여 API 호출을 보냅니다."]'::jsonb,'{"correct":3}'::jsonb),
    ('16be21c30acd55fd79f122b9b2647471','["WSGI는 요청마다 워커 스레드를 블로킹하는 동기 방식의 호출만 지원하며, 비동기·WebSocket 같은 프로토콜은 애초에 처리할 수 없는 구조다.","ASGI는 이름과 달리 실제로는 WSGI와 동일하게 동기 방식의 호출만 지원하고 비동기 프로토콜은 다루지 못한다.","WSGI는 HTTP 요청 응답 프로토콜을 사용하며, ASGI는 HTTP 및 WebSocket 연결을 동시에 처리할 수 있다.","ASGI는 WSGI와 마찬가지로 HTTP 요청·응답 프로토콜만 지원하며 WebSocket 같은 장수명 연결은 다루지 못한다."]'::jsonb,'{"correct":2}'::jsonb),
  ...
  ) AS v(content_md5, old_options, old_answer_key)
  JOIN question_bank q
    ON md5(q.content) = v.content_md5
   AND q.options = v.old_options
   AND q.answer_key = v.old_answer_key;
  IF matched <> 0 THEN
    RAISE EXCEPTION
      'question_bank accuracy correction incomplete: % stale defective rows remain',
      matched;
  END IF;
END
